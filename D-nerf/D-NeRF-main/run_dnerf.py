...
device = torch.device("cuda" if torch.cuda.is_available() else "cpu") # 千万别CPU，卡到死
np.random.seed(1)
DEBUG = False
...
def render_path(render_poses, render_times, hwf, chunk, render_kwargs, gt_imgs=None, savedir=None,
    ...
    if savedir is not None:
        save_dir_estim = os.path.join(savedir, "estim")  # 跑出来到结果
        save_dir_gt = os.path.join(savedir, "gt")   # ground truth
    ...
    for i, (c2w, frame_time) in enumerate(zip(tqdm(render_poses), render_times)):
        ...
        if savedir is not None:  # 如果路径不存在
...
def create_nerf(args):
    ...
    # 3D位置编码最大频率的log，默认10  ，默认0，为默认的位置编码
...
def config_parser():
    ...
    parser.add_argument("--dataset_type", type=str, default='llff',
                        help='options: llff / blender / deepvoxels')  # llff是lego
...
def train():
    ...
    summary_dir = '/root/autodl-tmp/dnerf/summaries/eval'
    ...
    if args.dataset_type == 'blender':
        ...
        save_path='/root/autodl-tmp/dnerf/data/nerf_synthetic/lego/train'
    ...
    for i in trange(start, N_iters):
        ...
        if use_batching:
            ...
            if i >= args.precrop_iters_time:
                ...
                max_sample = max(int(skip_factor), 3) #取出旋转平移矩阵？
            ...
            if N_rand is not None:
                ...
                if i < args.precrop_iters:
                    ...
                    coords = torch.stack(
                        ...
                        ), -1)  # 相当于生成了一堆坐标 一个小正方形从左上角到右下角的坐标
                    if i == start:
