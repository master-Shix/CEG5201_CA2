...
def process_batch(*,
  ...
  if 'rgb' in batch:
    ...
    lpips_alex = utils_pips.rgb_lpips(np.array(rgb), np.array(rgb_target), net_name='alex', device="cpu")
    lpips_vgg = utils_pips.rgb_lpips(np.array(rgb), np.array(rgb_target), net_name='vgg', device="cpu")
    out['lpips_alex'] = lpips_alex
    out['lpips_vgg'] = lpips_vgg
...
def process_iterator(tag: str,
  ...
  for i, (item_id, batch) in enumerate(zip(item_ids, iterator)):
    ...
    # # save_dir.mkdir(parents=True, exist_ok=True)
    # image_utils.save_image(f"/home/jiawei/weijia/literature/ourModels/nerfies_1stage/lego_4views_no_floor_final/renders/gt_{i}.png",
    #                        image_utils.image_to_uint8(batch['rgb']))
    # input()
    ...
    psnrs = jnp.append(psnrs,stats['psnr'])
    ssims.append(stats['ssim'])
    total_time +=stats['time']
    # ssims = jnp.append(ssims,stats['ssim'])
    lpips_alexes = jnp.append(lpips_alexes,stats['lpips_alex'])
    lpips_vgg = jnp.append(lpips_vgg,stats['lpips_vgg'])
  ...
  ssim = np.array(ssims)
  ssim = np.mean(ssim)
  lpips_alexes = jnp.mean(lpips_alexes)
  lpips_vgg = jnp.mean(lpips_vgg)
