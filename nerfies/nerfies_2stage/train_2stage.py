...
def main(argv):
    ...
    for step in range(start, train_config.max_steps + 1):
        ...
        # pytype: disable=attribute-error
        scalar_params = scalar_params.replace(
            learning_rate=learning_rate_sched(step-train_config.max_steps),
            elastic_loss_weight=elastic_loss_weight_sched(step-train_config.max_steps))
        warp_alpha = jax_utils.replicate(warp_alpha_sched(step-train_config.max_steps), devices)
        time_alpha = jax_utils.replicate(time_alpha_sched(step-train_config.max_steps), devices)
        state = state.replace(warp_alpha=warp_alpha, time_alpha=time_alpha)
