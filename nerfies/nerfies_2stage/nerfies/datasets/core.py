...
def load_camera(camera_path,
  ...
  if camera_path is not None:
    camera_path = gpath.GPath(camera_path)
  else:
    camera_path = None
  camera = cam.Camera.from_json(camera_path, item_id)
...
class DataSource(abc.ABC):
  ...
  def create_dataset2(self,
    ...
    """Creates a tf.data.Dataset."""
    if self.preload:
      return self._create_preloaded_dataset2(
        item_ids, flatten=flatten, shuffle=shuffle)
    else:
      return self._create_lazy_dataset(
        item_ids,
  ...
  def _create_preloaded_dataset2(self, item_ids, flatten=False, shuffle=False):
  ...
  def _create_preloaded_dataset(self, item_ids, flatten=False, shuffle=False):
    ...
    """
    load_fn = functools.partial(self.get_item)
    data_list = utils.parallel_map(load_fn, item_ids,0.5)
    data_list = [_camera_to_rays_fn(item) for item in data_list]
    data_dict = utils.tree_collate(data_list)
    return data_dict
    ...
    for i in range(len(item_ids)):
      ...
      s = []
      for i in select_coords:
        s.append(i[0].item() * H + i[1].item())
      select_inds_all.append(np.array(s))
    select_inds_all = np.array(select_inds_all)

    # select_inds = np.random.choice(400 * 400, size=[1024], replace=False)
    ...
    for key, value in data_dict.items():
      ...
      # if key =="rgb":
      #   n=0
      #   for i in out_dict[key][:1024]:
      #     if i[0]==1. and i[1]==1. and i[2]==1.:
      #       n+=1
      #   print(n)
      #   print(out_dict[key])
      #   print(np.sum(out_dict[key][:1024]))
      #   print(np.sum(out_dict[key][1024:2048]))
      #   print(out_dict[key][:1024].shape)
      #   input()
  ...
  def get_item(self, item_id, scale_factor=1.0):
    ...
    if scale_factor != 1.0 and rgb is not None:
    ...
    if self.use_appearance_id:
      ...
      data['metadata']['appearance'] = (self.get_appearance_id(item_id))
    if self.use_camera_id:
      # data['metadata']['camera'] = (
      #     self.camera_ids.index(self.get_camera_id(item_id)))
      data['metadata']['camera'] =(self.get_camera_id(item_id))
    if self.use_warp_id:
      # data['metadata']['warp'] = self.warp_ids.index(self.get_warp_id(item_id))
      data['metadata']['warp'] = (self.get_warp_id(item_id))
      # data['metadata']['warp'] = -1
    if self.use_time:
