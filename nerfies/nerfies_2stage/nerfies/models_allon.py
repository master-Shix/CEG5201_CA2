...
class NerfModel(nn.Module):
    ...
    def start_static_training(self, training, warp_ids=None, camera_ids=None, appearance_ids=None):
        self.start_train_static = training
        # self.use_warp = not training
        # self.use_warp_jacobian = not training
        # self.use_camera_metadata = not training
        # if not training:
        #     self.warp_ids = warp_ids
        #     self.camera_ids  = camera_ids
        #     self.appearance_ids = appearance_ids
    ...
    def render_samples(self,
        ...
        out = {}
        print("start render_samples")
        if return_points:
            out['points'] = points
        # Apply the deformation field to the samples.
        # if use_warp and not self.start_train_static:
        if use_warp :
            ...
            warp_out = self.warp_field(
                ...
                use_warp_jacobian,
                metadata_encoded)
            # warp_out = jax.lax.stop_gradient(warp_out)
            # if self.start_train_static:
            #     warp_out = jax.lax.stop_gradient(warp_out)
        ...
        # if not self.start_train_static:
        #     raw = jax.lax.stop_gradient(raw)
        # raw = jax.lax.stop_gradient(raw)
