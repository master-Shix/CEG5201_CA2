...
def main(argv):
  ...
  # model_config.use_warp = False
  # model_config.use_camera_metadata = False
  # model_config.use_elastic_loss = False
  # train_config.use_elastic_loss = False
  ...
  # print("print model")
  # print(state.optimizer.target['model'])
  # input()
  scalar_params = training.ScalarParams(
  ...
  model.start_static_training(True)
  train_config.use_elastic_loss = False
  ...
  # print(state.optimizer.target['model']["warp_field"]['trunk']['hidden_0'])
  # print(state.optimizer.target['model'])
  # input()
  # nerf_fine_params = state.optimizer.target['model']['nerf_mlps_fine'].unfreeze()
  # state.optimizer.replace(target={})
  # print(" make zero")
  # for key,value in state.optimizer.target['model'].items():
  #     print(key)
  # input()
  nerf_fine_params = state.optimizer.target['model']["nerf_mlps_fine"]
  nerf_coarse_params = state.optimizer.target['model']["nerf_mlps_coarse"]
  warp_params = state.optimizer.target['model']["warp_field"]
  ...
  for step, batch in zip(range(init_step, train_config.max_steps + 1),
    ...
    # dic = state.optimizer.target
    # dicModel = dic['model'].unfreeze()
    # dicModel['warp_field'] = warp_params
    # dicModel = flax.core.freeze(dicModel)
    # dic['model'] = dicModel
    # state.optimizer.replace(target=dic)

    # state.optimizer.target['model']["nerf_mlps_fine"] = state.optimizer.target['model']["nerf_mlps_fine"].unfreeze()
    # state.optimizer.target['model']["nerf_mlps_fine"] = nerf_fine_params
    # print(state.optimizer.target['model']["nerf_mlps_fine"]['MLP_0']['hidden_0'].unfreeze())
    # print(state.optimizer.target['model']["nerf_mlps_fine"]['MLP_0']['hidden_0'])
    # input()
    # state.optimizer.target['model']["nerf_mlps_fine"].copy(nerf_fine_params)
    # print(state.optimizer.target['model']["nerf_mlps_fine"]['MLP_0']['hidden_0'])
    # input()
    # state.optimizer.target['model']["nerf_mlps_coarse"] = nerf_fine_params
    ...
    with time_tracker.record_time('train_step'):
      ...
      # print(stats['coarse']['loss/rgb'])
      # print(stats['fine']['loss/rgb'])
      # print(stats['coarse']['predict/rgb'])
      # print(stats['fine']['predict/rgb'])
      # b = np.array(stats['fine']['predict/rgb'])
      # print(b.shape)
      # print(np.sum(b))
      # print(stats['grad'])
      # input()
    ...
    if step % train_config.print_every == 0 and jax.process_index() == 0:
      ...
      # print(stats['grad']['model']['warp_field'])
      # input()
      # print(state.optimizer.target['model']["warp_field"]['trunk']['hidden_0'])
      # input()
      # print(state.optimizer.target['model']['nerf_mlps_fine']['MLP_0']['hidden_0'])
      # print(stats['grad']['nerf_mlps_fine'])
      # for key,value in stats['grad']['model']['nerf_mlps_fine']['MLP_0'].items():
      #     print(key)
      #     print(value)
      # input()
  ...
  datasource_spec = {
        'type': exp_config.datasource_type,
        'data_dir': FLAGS.data_dir_dynamic,
  }
  logging.info('Creating datasource: %s', datasource_spec)
  ...
  print("start to train dynamic dataset")
  model.start_static_training(False)
  train_config.use_elastic_loss = True
  ...
  nerf_coarse_params = state.optimizer.target['model']["nerf_mlps_coarse"]
  warp_params = state.optimizer.target['model']["warp_field"]
  camera_params = state.optimizer.target['model']["camera_encoder"]


  init_step = train_config.max_steps
  allStep = train_config.max_steps+80000+1
  for step, batch in zip(range(init_step, allStep),
                         train_iter):


    # dic = state.optimizer.target
    # dicModel = dic['model'].unfreeze()
    # dicModel['nerf_mlps_fine'] = nerf_fine_params
    # dicModel['nerf_mlps_coarse'] = nerf_coarse_params
    # dicModel['camera_encoder'] = camera_params
    # dicModel = flax.core.freeze(dicModel)
    # dic['model'] = dicModel
    # state.optimizer.replace(target=dic)
    ...
    # print(state.optimizer.target['model']['nerf_mlps_fine']['MLP_0']['hidden_0'])
    # input()
    # print(state.optimizer.target['model']['warp_field'])
    # input()
    ...
    scalar_params = scalar_params.replace(
        learning_rate=learning_rate_sched(step),
        elastic_loss_weight=elastic_loss_weight_sched(step))
    warp_alpha = jax_utils.replicate(warp_alpha_sched(step-init_step), devices)
    time_alpha = jax_utils.replicate(time_alpha_sched(step), devices)
    state = state.replace(warp_alpha=warp_alpha, time_alpha=time_alpha)
    ...
    if step % train_config.print_every == 0 and jax.process_index() == 0:
      logging.info('step=%d, warp_alpha=%.04f, time_alpha=%.04f, %s', step,
                   warp_alpha_sched(step-init_step), time_alpha_sched(step),
                   time_tracker.summary_str('last'))
      coarse_metrics_str = ', '.join(
          [f'{k}={v.mean():.04f}' for k, v in stats['coarse'].items()])
      fine_metrics_str = ', '.join(
      ...
      # print(state.optimizer.target['model']['nerf_mlps_fine']['MLP_0']['hidden_0'])
      # input()
      # print(stats['grad']['model']['warp_field']['metadata_encoder']['embed']['embedding'])
      # # for key, value in stats['grad']['model']['warp_field']['metadata_encoder']['embed']['embedding'].items():
      # #       print(key)
      # #       print(value)
      # input()
