...
def main(argv):
  ...
  for step in range(start, train_config.max_steps+1):
      ...
      if step >= 10000:
          ...
          skip_factor = step / float(10000) * len(data_dic['rgb'])
      ...
      if step < 5000:
          ...
          coords = torch.stack(
              torch.meshgrid(
                  torch.linspace(H // 2 - dH-50, H // 2 + dH - 1, 2 * dH),
      ...
      # print(data_dic['origins'][img_i].shape)
      # print(data_dic['rgb'][img_i].shape)
      # print(select_coords[:, 0])
      ...
      appearance_id = data_dic['metadata']['appearance'][img_i]
      ...
      for i in range(N_rand):
          ...
          appearance.append(appearance_id)

      warps = np.array(warps)
      time = np.array(time)
      camera = np.array(camera)
      appearance = np.array(appearance)
      warps = warps[np.newaxis, :]
      time = time[np.newaxis, :]
      camera = camera[np.newaxis, :]
      appearance = appearance[np.newaxis, :]
      batch = {}
      batch['rgb'] = target_s
      ...
      batch['metadata']['appearance'] = appearance
