...
class DataSource(abc.ABC):
  ...
  def _create_preloaded_dataset(self, item_ids, flatten=False, shuffle=False):
    ...
    for i in select_coords:
      s.append(i[0].item() * 400 + i[1].item())
    select_inds = np.array(s)
    ...
    def _prepare_array(x):
      ...
      # x = np.concatenate([x.reshape(400, -1, x.shape[-1]) for x in x], axis=0)
      # x[,:,] = x[,:,][select_inds]
      x= x[...,select_inds,:]
  ...
  def _create_preloaded_dataset2(self, item_ids, flatten=False, shuffle=False):
    ...
    for key, value in data_dict.items():
      out_dict[key] = jax.tree_map(_prepare_array, value)
    # print("print dict")
    # print(out_dict['rgb'].shape)
    # print(out_dict['directions'].shape)
    # input()
